...
Допустимые фильтры: category, scenario, direction, team, agent, time range и finding type — только если поле доступно. Активные filters видимы, сериализуемы в URL и сбрасываются явно. Малые группы могут подавляться security policy.
